"""Run Alpha V5-Only Experiment: Alpha V5 with Kalman Filter Prediction

Alpha V5 extends V4 with Kalman Filter-based predictive decision making:
- Kalman Filter prediction (3 steps ahead = 15 seconds)
- OR logic: trigger if current OR predicted values meet condition
- Adaptive stable_evals: reduce from 5→2 when velocity is high (rapid change)
- Enhanced diagnostics for prediction analysis
"""
...
def _run_one(args: Tuple) -> Dict[str, Any]:
    ...
    print(f"  - Alpha V5 decode→prefill: {config.alpha_v5_allow_decode_to_prefill}", flush=True)
    print(f"  - Alpha V5 prefill→decode: {config.alpha_v5_allow_prefill_to_decode}", flush=True)
...
def experiment_alpha_v5_only(max_workers: Optional[int] = None) -> List[Dict]:
    """Test alpha_v5 policy only x 3 offload modes with 1h Azure code requests.
    ...
    - alpha_v5_allow_decode_to_prefill = True (allow decode → prefill)
    - alpha_v5_allow_prefill_to_decode = False (disable prefill → decode)

    Alpha V5 uses average decode memory guard and p75 alpha for more robust switching.
    ...
    Policies tested:
    - alpha_v5: Alpha V5 policy with no offload
    - alpha_v5_offload: Alpha V5 policy with offload configurations
    ...
    """
    print("\n" + "="*80)
    print("=== Alpha V5-Only Experiment (1h trace): Baselines + Alpha V5 + Alpha V5 with Offload ===")
    print("=== CRITICAL: Only decode→prefill switching allowed (prefill→decode disabled) ===")
    print("=== Alpha V5 uses average decode memory and p75 alpha for robust switching ===")
    print("="*80 + "\n")
    ...
    # Only test alpha_v5 policy (no other policies)
    policies = [
        ("alpha_v5", True, "alpha_v5"),
    ]
    print(f"\n[CONFIG] Testing alpha_v5 policy only")
    ...
    print(f"[CONFIG] IMPORTANT: alpha_v5_allow_decode_to_prefill=True in ALL configs")
    print(f"[CONFIG]            alpha_v5_allow_prefill_to_decode=False in ALL configs")
    ...
    for policy_name, enable_sw, sw_policy in policies:
        ...
        config = SimConfig(
            ...
            alpha_v5_allow_decode_to_prefill=True,
            alpha_v5_allow_prefill_to_decode=True,
            # Kalman Filter params for Alpha V5
            kf_process_noise=0.01,
            kf_measurement_noise=0.1,
            kf_prediction_steps=3,
            kf_velocity_threshold=0.05,
            kf_min_stable_evals=2,
            enable_dynamic_lp=False,
            # Enable streaming loading to avoid OOM
            enable_streaming_loading=True,
            streaming_window_size=300.0,  # 5 minutes
...
def main():
    import sys
    print("\n" + "="*80)
    print("ALPHA V5-ONLY EXPERIMENT (1H TRACE): ALPHA V5 POLICY WITH ONLY DECODE→PREFILL")
    print("="*80)
    ...
    print(f"[INIT] Starting alpha_v5-only experiment (1h trace)...")
    results = experiment_alpha_v5_only(max_workers=workers)
    save_experiment("alpha_v5_kf_prediction", results)
