"""Run Alpha-Only Experiment: Alpha + Alpha with Offload (no decode→prefill switching)"""
...
def experiment_alpha_only_4p4d(max_workers: Optional[int] = None) -> List[Dict]:
    ...
    Key difference: alpha_allow_decode_to_prefill is set to False, so decode cannot switch to prefill.
    Only prefill→decode switching is allowed.
    ...
    """
    print("\n" + "="*80)
    print("=== Alpha-Only Experiment (1h trace): Alpha + Alpha with Offload (6 configurations) ===")
    print("=== CRITICAL: alpha_allow_decode_to_prefill=False (decode→prefill disabled) ===")
    print("="*80 + "\n")
    ...
    print(f"[CONFIG] IMPORTANT: alpha_allow_decode_to_prefill=False in ALL configs")
    print(f"[CONFIG]            This means decode CANNOT switch to prefill\n")
    ...
    for policy_name, enable_sw, sw_policy in policies:
        ...
        config = SimConfig(
            ...
            alpha_allow_decode_to_prefill=False,  # CRITICAL: Disable decode→prefill
...
def main():
    import sys
    print("\n" + "="*80)
    print("ALPHA-ONLY EXPERIMENT (1H TRACE): ALPHA POLICY WITHOUT DECODE→PREFILL")
    print("="*80)
