...
def _ensure_azure_24h() -> Path:
    """Ensure the 24-hour Azure code trace exists."""
    print(f"[TRACE] Checking for 24h Azure trace at {AZURE_TRACE_24H}...")
    if AZURE_TRACE_24H.exists():
        print(f"[TRACE] ✓ Found trace file")
        return AZURE_TRACE_24H

    raise FileNotFoundError(
        f"24-hour Azure trace not found at {AZURE_TRACE_24H}. "
        "Please run extract_24h_trace.py first to create it."
    )


def experiment_switching_offload_4p4d(max_workers: Optional[int] = None) -> List[Dict]:
    """Test 4 switching policies x 3 offload modes at 4p4d with 24h Azure code requests.
    ...
    """
    print("\n" + "="*80)
    print("=== Experiment 7: Switching + Offload at 4p4d (12 configurations) ===")
    print("="*80 + "\n")
    trace = str(_ensure_azure_24h())
    ...
    for policy_name, enable_sw, sw_policy in policies:
        ...
        config = SimConfig(
            ...
            # Enable streaming loading for 24h trace to avoid OOM
...
def main():
    import sys
    print("\n" + "="*80)
    print("EXPERIMENT 7: SWITCHING + OFFLOAD INTERACTION")
    print("="*80)
