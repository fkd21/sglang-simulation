...
class PolicyAlphaSimV2(object):
    ...
    Direction:
      ...
      - if max_prefill_alpha < alpha_threshold AND decode_pressure > decode_high
    ...
    def evaluate(self, now_unix: float, states: Dict[int, WorkerState]) -> Dict[str, Any]:
        ...
        dp_condition = (max_prefill_alpha > alpha_threshold) and (decode_pressure < float(self.args.decode_low))
        pd_condition = (max_prefill_alpha < alpha_threshold) and (decode_pressure > float(self.args.decode_high))
